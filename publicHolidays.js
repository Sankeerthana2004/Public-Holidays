// newholiday.js
// Enhances existing UI behavior without modifying design:
// - fetches AvailableCountries -> populates dropdown
// - accepts manual country code input (priority over dropdown)
// - supports year & month selection
// - caches countries & holidays in localStorage
// - groups holidays by month into collapsible sections (cards inside table-wrapper)
// - images from Unsplash + fallback
// - robust loading/error handling
// - keeps existing modal helpers intact

/* DOM nodes (existing elements) */
...
function qLog(...args){ console.log(...args); }
...
function setError(message){
  ...
  error.innerHTML = `<p>${message || "Sorry, we couldn't load the holidays. Please try again."}</p>`;
}
...
function saveCountriesToCache(data){
  try { localStorage.setItem(COUNTRIES_KEY, JSON.stringify({ts: Date.now(), data})); } catch(e){/* ignore */ }
}
function loadCountriesFromCache(maxAgeMs = 24*60*60*1000){ // 24h
  try{
...
function cacheHolidays(code, year, holidays){
  try { localStorage.setItem(`${CACHE_PREFIX}${code}_${year}`, JSON.stringify({ts: Date.now(), holidays})); } catch(e){/* ignore */ }
}
function loadCachedHolidays(code, year, maxAgeMs = 6*60*60*1000){ // 6 hours
  try{
...
function populateCountrySelect(list){
  ...
  if (locale){
    const found = [...countrySelect.options].find(o => o.value === locale.toUpperCase());
...
function createHolidayCard(h, countryCode){
  ...
  const typesText = Array.isArray(h.types) && h.types.length ? h.types.join(', ') : 'N/A';
  const regionText = (h.counties && h.counties.length) ? `Applies to: ${h.counties.join(', ')}` : (h.global ? 'Nationwide' : 'Regional');
  metaP.innerHTML = `<strong>Type:</strong> ${typesText} • ${regionText}${h.launchYear ? ` • Since ${h.launchYear}` : ''}`;
  ...
  let desc = `Observed on ${new Date(h.date + 'T00:00:00').toLocaleDateString(undefined, { month:'long', day:'numeric', year:'numeric' })}. `;
  if (h.localName && h.localName !== h.name) desc += `Locally known as ${h.localName}. `;
...
function renderGrouped(holidays, filterMonth='all', countryCode){
  ...
  if (filterMonth !== 'all'){
    filtered = holidays.filter(h => (new Date(h.date + 'T00:00:00')).getMonth() + 1 === Number(filterMonth));
  }
  ...
  grouped.forEach(([monthNum, items]) => {
    ...
    header.style.background = 'linear-gradient(90deg, rgba(255,255,255,0.98), rgba(245,245,245,0.98))';
...
async function fetchHolidaysFor(code, year){
  ...
  try{
    const url = `${API_BASE}/PublicHolidays/${encodeURIComponent(year)}/${encodeURIComponent(code)}`;
    qLog('Fetching', url);
    ...
    setLoading(false);
    setError('Could not load holidays. Please check the country code or try again later.');
  }
}
...
async function handleSearch(){
  ...
  const manual = (countryInput && countryInput.value && countryInput.value.trim()) ? countryInput.value.trim().toUpperCase() : '';
  const dropdown = (countrySelect && countrySelect.value) ? countrySelect.value : '';
  const countryCode = manual || dropdown;
  const year = (yearInput && yearInput.value) ? yearInput.value : (new Date().getFullYear());
  const month = (monthSelect && monthSelect.value) ? monthSelect.value : 'all';

  if (!countryCode){
    setError('Please choose a country from the dropdown or enter a 2-letter country code.');
    return;
  }
...
function initialize(){
  ...
  if (countryInput) countryInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') handleSearch(); });
  if (yearInput) yearInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') handleSearch(); });
  if (countrySelect) countrySelect.addEventListener('keydown', (e) => { if (e.key === 'Enter') handleSearch(); });
  ...
  if (aboutBtn) aboutBtn.addEventListener('click', () => showModal('aboutModal'));
